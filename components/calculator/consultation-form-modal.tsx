...
import { Loader2, CheckCircle, Mail, Upload, X, Plus, Trash2 } from 'lucide-react';
...
export function ConsultationFormModal({
  ...
  const [hairStyleImagePreview, setHairStyleImagePreview] = useState<string | null>(null);
  const [currentHairImagePreview, setCurrentHairImagePreview] = useState<string | null>(null);
  const [currentProducts, setCurrentProducts] = useState<Array<{productName: string; purpose: string}>>([]);
  const [medications, setMedications] = useState<Array<{medicationName: string; purpose: string}>>([]);
  ...
  const handleHairStyleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleCurrentHairImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
  ...
  const updateCurrentProduct = (index: number, field: 'productName' | 'purpose', value: string) => {
  ...
  const updateMedication = (index: number, field: 'medicationName' | 'purpose', value: string) => {
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="section">
              ...
              ${formData.occupation ? `<div class="field">
                <div class="label">Occupation:</div>
                <div class="value">${formData.occupation}</div>
              </div>` : ''}
              ${formData.dateOfBirth ? `<div class="field">
            ...
            ${formData.currentHairImage ? `<div class="section">
            ...
            ${formData.pastTreatments && formData.pastTreatments.length > 0 ? `<div class="section">
            ...
            ${formData.currentProducts && formData.currentProducts.length > 0 ? `<div class="section">
              <div class="section-title">Current Hair Products</div>
              <div class="product-list">
                ${formData.currentProducts.map((product: any, index: number) => `
            ...
            ${formData.medications && formData.medications.length > 0 ? `<div class="section">
            ...
            ${formData.referralSource && formData.referralSource.length > 0 ? `<div class="section">
            ...
            ${formData.specialInstructions ? `<div class="section">
            ...
            <div class="section">
              ...
              ${formData.minorTermsAccepted ? `<div class="field">
                <div class="label">Minor Terms Accepted:</div>
                <div class="value">Yes</div>
              </div>` : ''}
              ${formData.signature ? `<div class="field">
                <div class="label">Signature:</div>
                <div class="value">${formData.signature}</div>
              </div>` : ''}
              ${formData.dateSigned ? `<div class="field">
    ...
    try {
      ...
      Object.keys(values).forEach(key => {
        ...
        if (typeof values[key as keyof typeof values] === 'object') {
          formData.append(key, JSON.stringify(values[key as keyof typeof values]));
        } else {
      ...
      try {
        ...
        console.log('Database save failed (optional), but email was sent:', dbError);
      }
      ...
      setEmailError(error.message || 'Failed to send consultation request. Please try again or contact us directly.');
    } finally {
        ...
        {isSuccess ? (
          <div className="py-12 text-center">
            ...
            <p className="text-gray-600 mb-4">
              ...
              Our team will contact you soon at the email you provided.
            </p>
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Fill out this comprehensive form to help us understand your hair needs and provide the best service.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="bg-emerald-50 rounded-lg p-4 mb-4">
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-700">Selected Hair Loss Pattern:</span>
                  <span className="text-sm font-semibold text-emerald-600">{selectedType}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-700">Estimated Grafts Required:</span>
                  <span className="text-sm font-semibold text-emerald-600">{estimatedGrafts}</span>
                </div>
                {estimatedPrice && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-700">Estimated Cost:</span>
                    <span className="text-sm font-semibold text-emerald-600">{estimatedPrice}</span>
                  </div>
                )}
                <div className="mt-2 text-xs text-gray-500">
                  This information will be included in the email to our support team.
                </div>
              </div>
            </div>
                      ...
                      <div key={index} className="flex gap-3 items-start p-3 border rounded-lg">
                        <div className="flex-1 space-y-2">
                          <Input
                            ...
                            onChange={(e) => updateCurrentProduct(index, 'productName', e.target.value)}
                          />
                          <Input
                            placeholder="Purpose of using this product"
                            value={product.purpose}
                            onChange={(e) => updateCurrentProduct(index, 'purpose', e.target.value)}
                          />
                        </div>
                        <Button
                          type="button"
